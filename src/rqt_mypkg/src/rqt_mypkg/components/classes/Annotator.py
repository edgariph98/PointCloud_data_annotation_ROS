...
class Annotator():
    ...
    def __init__(self, _rviz_frame, _frames):
        ...
        try:
            ...
            self.annotationCreatedPublisher = rospy.Publisher("/annotation_created",Marker,queue_size=1)
    ...
    def createAnnotation(
        ...
        if id in self.annotationIds:
            ...
            # the user hasmade aselection
            if self.current_bounding_box_selection:
                # creating new annotation
                newAnnotation = Annotation( id, labelName, groupName, self.current_bounding_box_selection, color)
                ...
                self.server.insert( newAnnotation.getInteractiveMarker(), self.processFeedback)
                self.server.applyChanges()
                # resetting the selection to None
                self.annotationCreatedPublisher.publish(self.current_bounding_box_selection)
                self.current_bounding_box_selection = None
